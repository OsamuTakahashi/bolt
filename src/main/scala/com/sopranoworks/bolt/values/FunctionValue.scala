...
case class FunctionValue(name:String,parameters:java.util.List[Value]) extends WrappedValue {
  ...
  private def _checkStayUnresolved:Boolean = {
    ...
    name match {
      ...
      case "$BETWEEN" =>
        ...
        if (!_checkStayUnresolved) {
          ...
          try {
            _ref = Some(BooleanExpressionValue("AND", BooleanExpressionValue("<=", a, v), BooleanExpressionValue("<=", v, b)).eval.asValue)
          } catch {
